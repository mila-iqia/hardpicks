...
from hardpicks.analysis.fbp.publication.path_constants import output_directory, pickles_directory
...
metrics_renaming_dictionary = {
    ...
    'valid/RootMeanSquaredError': ('DEL3', 'RMSE'),
    'valid/MeanAbsoluteError': ('DEL4', 'MAE'),
    'valid/MeanBiasError': ('DEL5', 'MBE'),
}
...
if __name__ == "__main__":
    ...
    summary_df = pd.merge(job_count_df, summary_df, left_index=True, right_index=True).reset_index()
    ...
    output_df = summary_df.rename(columns=metrics_renaming_dictionary)[desired_columns].reset_index(drop=True)
